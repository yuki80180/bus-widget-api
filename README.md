...
次発から最大3便、次発までの残り時間、系統、行き先、のりばを表示します。本日の最終便終了後は、最大7日先まで検索した次の運行日の始発を案内します。運行終了と通信エラーは専用表示に切り替わり、更新ボタンまたは画面表示中の1分ごとの自動取得で最新情報を確認できます。便の検索と残り時間の計算はAPI側で行い、ブラウザ側には同じ時刻表ロジックを持ちません。
